...
def test_sim_bkpt16():
    """This test does not use the opcode_factory because its instructions were
    transcribed directly from an ELF file.
    """
    instructions = [ (opcode_factory.bkpt16(), 16),  # BKPT16
                   ]
    epiphany = MockEpiphany()
    epiphany.init_state(instructions, pc=0)
...
def test_sim_nop16():
    ...
    assert epiphany.state.running
    epiphany.run()
    expected_state = StateChecker(pc=4)
    expected_state.check(epiphany.state)
    assert not epiphany.state.running
...
def test_sim_all_16bit():
    ...
    assert epiphany.state.running
    epiphany.run()
    expected_state = StateChecker(pc=8)
    expected_state.check(epiphany.state)
    assert not epiphany.state.running
